...
string xtime(string temps){//temp hex
    ...
    if(cc=='1'){
        //cout<"co";
        for(int i=0;i<=7;i++){
            if(temps[i]==tempbit[i]) temps[i]='0';
            else temps[i]='1';
        }
    }
    //cout<<"da "<<temps<<endl;
...
void getRcon(int i,string da[5]){//rcon vong thu i
    for(int j=1;j<=4;j++){
        da[j]=rcon[j][i];
    }
    return;
...
string Xorbin(string s1,string s2){// input 2
    string da=s1;
    for(int i=0;i<=7;i++){
        if(s1[i]==s2[i]) da[i]='0';
        else da[i]='1'; 
...
string invnhanmbox(string temps,int x){// return 2
    if(x==9){
        return 
Xorbin(Get2From16(xtime(xtime(xtime(temps)))),Get2From16(temps));
    ...
    if(x==11){
        return 
Xorbin(Xorbin(Get2From16(xtime(temps)),Get2From16(xtime(xtime(xtime(temps))))
),Get2From16(temps)) ; 
    }
    if(x==13){
        return 
Xorbin(Xorbin(Get2From16(xtime(xtime(temps))),Get2From16(temps)), 
Get2From16(xtime(xtime(xtime(temps)))));
    }
    if(x==14){
        return 
Xorbin(Xorbin(Get2From16(xtime(xtime(temps))),Get2From16(xtime(temps))), 
Get2From16(xtime(xtime(xtime(temps)))));
    }
    return temps;
}
...
void Nhap(){
    ...
    while(s.length()<16){//vi ma hoa 128 nen minh them cho du 16 ki tu
        s=s+" ";
    }
    cout<<s<<endl;
    int ii=0;
    ...
    while(sk.length()<16){// tuong tu nhu vay
        sk=sk+" ";
    }
    cout<<sk<<endl;
    ii=0;
...
void xuat(){
    cout<<endl<<"==============================="<<endl<<"text giai ma: "<<endl;
    for(int i=1;i<=4;i++){
        for(int j=1;j<=4;j++){
...
void addRoundKey(){
    ...
    for(int i=2;i<=4;i++){
        for(int j=1;j<=4;j++){
            cipherkey[j][i]=Get16From2(Xorbin(Get2From16(cipherkey[j][i-
1]),Get2From16(cipherkey[j][i])));
        }
    }
}
...
void mixColumn(){
    for(int j=1;j<=4;j++){
        ...
        temp[4]=state[4][j];
        string da[5];
        for(int i=1;i<=4;i++){//lay tung hang trong mbox
...
for(int i=4;i>=2;i--){
for(int j=1;j<=4;j++){
cipherkey[j][i]=Get16From2(Xorbin(Get2From16(cipherkey[j][i]),Get2From16
(cipherkey[j][i-1])));
}
}
//c1
...
init();
while(1){
cout<<endl<<"===========MENU========"<<endl<<"1. ma hoa"<<endl<<"2. giai ma"<<endl<<"0. exit"<<endl<<"lua chon: ";
int m;
cin>>m;
//fflush(stdin);
if(m==0) return 0;
